...
class MultiRobotPlannerConfig:
    ...
    time_weight: float = 1.0  # Weight for total-time minimization cost
    
    # Learnable dt bounds
    dt_min: float = 0.01   # Minimum allowed dt (prevents collapse to zero)
    dt_max: float = 1.0    # Maximum allowed dt
    dt_learnable: bool = True  # Whether dt is optimized during planning
...
class MultiRobotPathPlanner:
    ...
    def smoothness_cost(self, trajectory: torch.Tensor, dt: torch.Tensor = None) -> torch.Tensor:
        """
        Compute trajectory smoothness cost (velocity and acceleration).
        
        Args:
            trajectory: Full trajectory [timesteps, n_robots, 2]
            dt: Time step (scalar tensor if learnable, else uses config.dt)
    ...
    def time_cost(self, dt: torch.Tensor) -> torch.Tensor:
        """
        Cost that encourages minimizing total trajectory time.
        
        Total time = n_timesteps * dt. The optimizer will try to shrink dt
        to minimize this cost, while smoothness/velocity costs push back
        (since velocity ~ 1/dt, smaller dt means larger velocities).
        
        Args:
            dt: Learnable time step (scalar tensor)
            
        Returns:
            cost: Time minimization cost scalar
        """
        total_time = self.config.n_timesteps * dt
        return self.config.time_weight * total_time
    ...
    def total_cost(
        ...
        Args:
            ...
            start_positions: [n_robots, 2]
            goal_positions: [n_robots, 2]
            dt: Learnable time step tensor (uses config.dt if None)
        ...
        # Smoothness cost
        smooth_cost = self.smoothness_cost(trajectory, dt_val)
        ...
        if trajectory.shape[0] > 1:
            final_vel = (trajectory[-1] - trajectory[-2]) / dt_val
        ...
        # Add time minimization cost when dt is learnable
        if dt is not None:
            total = total + self.time_cost(dt)
    ...
    def plan(
        ...
        initial_dt = self.config.dt
        if self.config.dt_learnable:
            dt_param = torch.tensor(
                [initial_dt],
                device=self.config.device,
                dtype=self.config.dtype,
                requires_grad=True
            )
            opt_params = [trajectory, dt_param]
        else:
            dt_param = None
            opt_params = [trajectory]
        
        # Optimizer
        optimizer = torch.optim.Adam(opt_params, lr=self.config.learning_rate)
        ...
        for iteration in range(self.config.n_iterations):
            ...
            # Compute total cost
            cost = self.total_cost(trajectory, start_positions, goal_positions, dt=dt_param)
            ...
            if dt_param is not None and dt_param.grad is not None:
                torch.nn.utils.clip_grad_norm_([dt_param], max_norm=1.0)
        ...
        with torch.no_grad():
            ...
            final_dt = dt_param.item() if dt_param is not None else self.config.dt
        ...
        info = {
            ...
            'final_dt': final_dt,
            'total_trajectory_time': final_dt * n_t,
        }
        ...
        if verbose:
            ...
            if dt_param is not None:
                print(f"Learned dt: {final_dt:.4f} (initial: {initial_dt:.4f})")
                print(f"Total trajectory time: {info['total_trajectory_time']:.2f}s")
